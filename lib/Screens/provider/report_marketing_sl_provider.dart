...
import 'package:kreditpensiun_apps/Screens/models/report_marketing_model.dart';
...
class ReportMarketingSlProvider extends ChangeNotifier {
  ...
  Future<List<ReportMarketingModel>> getMarketingSlReport(
    ...
    if (response.statusCode == 200) {
      final result = json
          .decode(response.body)['Daftar_Report_Marketing_Sl']
          .cast<Map<String, dynamic>>();
      _data = result
          .map<ReportMarketingModel>(
              (json) => ReportMarketingModel.fromJson(json))
          .toList();
      return _data;
    } else {
      throw Exception();
