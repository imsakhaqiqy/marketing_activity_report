import 'package:kreditpensiun_apps/Screens/models/planning_model.dart';
...
class PlanningProvider extends ChangeNotifier {
  ...
  Future<List<PlanningModel>> getPlanning(PlanningItem planningItem) async {
    ...
    if (response.statusCode == 200) {
      final result = json
          .decode(response.body)['Daftar_Planning']
          .cast<Map<String, dynamic>>();
      _data = result
          .map<PlanningModel>((json) => PlanningModel.fromJson(json))
          .toList();
      return _data;
    } else {
      throw Exception();
